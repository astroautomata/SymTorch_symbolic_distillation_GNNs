    ...
    for epoch in range(num_epoch):
        ...
        if model_type == 'pruning':
           #update mask at specific epochs using val data
            # Don't pass parent_model to avoid forward() call issues with edge_index
        ...
        if model_type == 'pruning':
            ...
            log_dict['active_message_dims'] = model.edge_model.current_dim
...
def load_model(dataset_name, model_type, num_epoch):
    ...
    if model_type == 'pruning':
        # Load data into the registered buffer to maintain device consistency
